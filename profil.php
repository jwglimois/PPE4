...
<?php if (!empty($_SESSION['id'])) { ?>
    ...
    <?php if(!empty($isVendeur)){ ?>
        ...
        <p>Objets mis en vente : <?= $nbObjets; ?><br>
            ...
            Messages postés : <?= $nbComs; ?>
        </p>

        <h2>Messages posés</h2>
        <p>
        <table id="message">
            <tr>
                <th>Article</th>
            ...
            foreach($articleVente as $row){
                $commentaires=$pdo->query('select * from commentaire where id_article='.$row['id'])->fetchAll();

                foreach($commentaires as $row){
                    $articles=$pdo->query('select nomArticle , c.id_membre as acheteur, message, date  
                                                    from article a                                             
                                                    inner join commentaire c on a.id=c.id_article
                                                    where a.id='.$row['id_article'])->fetchAll(); 
                }                         
                                    
                foreach($articles as $article ){
                    $pseudoAcheteur=$pdo->query('select pseudo from membre where id='.$article['acheteur'])->fetch();
                    ?>
                <tr>
                    <td><?= $article['nomArticle']?></td>
                    <td><?= $article['message'] ?></td>
                    <td><?= $pseudoAcheteur[0] ?></td>
                    <td><?= $article['date'] ?></td>
                    </tr>
